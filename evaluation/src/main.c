/**############################################################################
 ** TITLE:			CipherSearch 3.0
 ** CONTRIBUTORS:	Magnus Overbo & Adrian S. Evensen
 ** ABOUT:			Ciphersearch utilises the GMP library to manage arbitrary sized
 ...
 **						generated by a decimated LFSR encrypted with a message, and
 **						tries to find this in all possible generated sequences which are
 **						not decimated. The candidate set can then be attacked with
 **						a brute force attack in order to break the cryptosystem.
 **
 ** Release:		20190313 - 64b version
 ** Release:		20190328 - GMP library version for arbitrary bit size
 ** Release:		20210531 - Add option for shift-OR implementation (default)
 **							 - Add brute force for clock reconstruction
 **#########################################################################**/
...
// GLOBAL VARIABLES
//-----------------------------------------------------------------------------
size_t num_threads = 16;	// Thread pool limit. Generally set to # of vCPUs in cloud instances
...
int polyMap(int);													// Get LFSR polynomial
...
// CSV Parse function fetched from 
...
void match_R1(void *arg) { // Thread for brute force attempts
	...
	while ( i<mpz_get_ui(max) && col <= col_acceptance ) { // Iterate through all R1States until a hit is found or collision acceptance exceeded
		#if defined DEBUG
			printf("Generating clocking LFSR (R1) output sequence: \n");
		#endif
		lfsrgen(LCLK, deg, m, pol, i, 0, NULL);				// Clocking LFSR, R1

		#if defined DEBUG
			printf("Calculating the Decimated bitsequence and creating ciphertext:\n\n");
		#endif
	
		genEncrypt(CIPHER2, LCLK, cand->X , PLAINTEXT, m); 	// Generate the candidate cipher

		if ( mpz_cmp(CIPHER, CIPHER2) == 0 ) {				// Check if candidate cipher matches
			#if defined DEMO
			printf("\t - POSSIBLE MATCH FOUND for R1 init state %i and R2 init state %i\n", i, cand->istate);
			col++; 
			#else
...
void search_thread(void *arg) { // Thread for ARBP search
  	struct CANDIDATE *cand = (struct CANDIDATE *)arg;
...
int main(int argc, char *argv[]){
	...
	mpz_init( pol );
	mpz_set_ui(pol, polyMap(deg));						//Get LFSR polynomial

	mpz_init( max );
	mpz_setbit(max, deg);								//Set max val, eg 2047 in 2^11
	...
	mpz_t R2SEQ;		mpz_init(R2SEQ);				//LFSR to be decimated
	lfsrgen(R2SEQ, deg, n, pol, R2STATE, 0, NULL);		//Decimated LFSR
...
while (true) {
	...
	struct CANDIDATE* endPtr = C + (sizeof(*C)/sizeof(struct CANDIDATE) * mpz_get_ui(max));
		...
		while (fgets(line, 1024, stream)) // iterate through all lines in .cand file
		{
			...
			C[l].istate = l;
			mpz_set_str(C[l].X, getfield(tmp2, 2), 62); //Decode the base-62 sequence
		}
		...
		for (int i = 0; i < (mpz_get_ui(max)-1); i++){		// Iterate through all initial states of R2
			C[i].istate = i+1;
			tpool_add_work(tm, search_thread, &C[i]);		// Launch thread for ARBP
		}
		tpool_wait(tm);										// Wait here until all threads are dead
		tpool_destroy(tm);
		...
		while ( ptr < endPtr ) { // Iterate through all possible candidates
			if (ptr->match == true){	// If this is a valid candidate
				fprintf(fh, "\n%i,", ptr->istate); mpz_out_str(fh, 62, ptr->X); // Write it to file
				#if !defined DEMO				
					if (ptr->istate == R2STATE) {
						found = 1;
					}
				#endif
				u++;	// Increase candidate counter
			}
			ptr++;
		}
		...
		if ( u == 0 ) { // Zero matches, empty dataset
			remove(FNAME); // Discard file
			#if defined DEMO
				printf("[k=%d] Empty candidate set. Increasing error threshold\n", slen);
				slen++;
			#else
				printf("%d,%d",-1,u);
				return -1; 
			...
			remove(FNAME); // Discard file
			#if defined DEMO
				printf("[k=%d] Full candidate set. Decreasing error threshold\n", slen);
				slen--;
			#else
				printf("%d,%d",-2,u);
				return -2;
			...
			tpool_wait(tm);	// Wait here until brute force is done
			tpool_destroy(tm);
			...
			if (col>0) {
				...
				printf("A match was found in %ld! Exiting.\n", micros);
				return 0;
			} else {
				printf("No matches found... Increasing error threshold \n");
				slen++;
			}
			#else
			if (col > 0) {
...
mpz_t* arbp_search(mpz_t* B, mpz_t TEXT, int K, int m, int n) {
	...
	#if defined CONSTRAINTS2 // Additional matrix for the OR3 algorithm
	...
	while( pos < n ){									//Search entire TEXT
		...
		while( i < K ) {
			...
			mpz_init(tmp1);	mpz_init(tmp2);mpz_init(tmp3); //Reset and initialise temp variables
			...
			#if defined SHIFTOR
				#if defined CONSTRAINTS2 // OR3 algorithm implementation
				// Deletion 
				mpz_set(tmp2, oldR);
				mpz_lshift(tmp2, m);
				...
				#elif defined CONSTRAINTS1 // OR2 algorithm implementation
				// Deletion  
				mpz_lshift(newR, m);
				...
				#else // OR1 algorithm implementation
				mpz_set(tmp1, oldR);
